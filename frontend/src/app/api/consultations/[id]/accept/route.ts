  ...
  try {
    ...
    if (consultationId.startsWith('sample-')) {
      return NextResponse.json({
        id: consultationId,
        status: 'scheduled',
        message: '상담 요청을 수락했습니다.',
      });
    }

    const response = await fetch(`${BACKEND_URL}/api/consultations/${consultationId}/accept`, {
      method: 'POST',
      headers: {
    ...
    console.error('[API Route] Consultation Accept error:', error);
    return NextResponse.json(
      { message: '상담 수락 중 오류가 발생했습니다' },
      { status: 500 }
    );
  }
}
