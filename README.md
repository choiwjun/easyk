...
프론트엔드 페이지에서 API 연결 없이 샘플 데이터로 UI를 테스트할 수 있습니다.

#### 1. 일자리 페이지 (`/jobs`)

- **토글 버튼**: 페이지 상단의 노란색 토글 버튼 클릭
- **🟢 샘플 데이터 중**: 6개 샘플 일자리 공고 표시
- **⚪ API 사용 중**: 실제 백엔드 API 호출

#### 2. 정부 지원 페이지 (`/supports`)

- **토글 버튼**: 페이지 상단의 노란색 토글 버튼 클릭
- **🟢 샘플 데이터 중**: 5개 샘플 정부 지원 프로그램 표시
- **⚪ API 사용 중**: 실제 백엔드 API 호출
...
- 도시 계획 연구원 (서울특별시)
- 외국인 상담 지원사 (한국이주재단)
- 제조업 생산 현장 인력 (자동차 부품 제조사 A)
- 호텔 프론트 데스크 (다국어) - 호텔 B
- 식음료 배달 및 물류 지원 - 물류 센터 C

#### 정부 지원 샘플 (5개)
- 외국인 장려금 지원사업 (장려금)
- 다문화 가정 아동 한국어 교육 지원 (교육)
- 외국인 취업 성장 지원 사업 (훈련)
- F-2 비자 체류 기간 연장 지원 (비자/체류)
- 새터민 청소년 입주자 임대 주택 지원 (주거)

### 언제 샘플 데이터를 사용하나요?

- ✅ **개발 단계**: UI 컴포넌트 개발 시
- ✅ **테스트**: UI 디자인 및 사용자 경험 테스트
- ✅ **프레젠테이션**: 백엔드 없이 프레젠테이션 가능
- ✅ **API 오류 발생 시**: 자동으로 샘플 데이터로 폴백
...
샘플 데이터는 `sampleData.ts` 파일에서 수정할 수 있습니다.
